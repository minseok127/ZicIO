...
The original ZicIO used a 4MB buffer to cache ext4_extent. My initial focus was on avoiding extra resource overhead over the previous implementation. So I started creating a mapping table while maintaining this amount of memory. The mapping table consists of two arrays. One is an array of structures containing the minimal information needed to create a single NVMe command. The other specifies which parts of that structure are required for each 2MB huge page that makes up the buffer. Let's calculate how much I/O can be covered when these lists are limited to 4MB in size.
