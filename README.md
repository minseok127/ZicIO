...
The original ZicIO used a 4MB buffer to cache ext4_extent. My initial focus was on avoiding extra resource overhead over the previous implementation. So I started creating a mapping table while maintaining this amount of memory. The mapping table consists of two arrays. One is an array of data structure containing the minimal information needed to create a single NVMe command, each element corresponds to each read I/O. The other specifies which parts of that structure are required for each 2MB huge page that makes up the buffer. When creating I/O commands for a 2MB huge page, the range of offsets listed in this array is used to access the elements of the aforementioned list.
...
In summary, 8 bytes of compressed information can be used to create a read I/O command ranging from 4KB to 256KB. Naively, in the worst case, 4MB can cover 2GB of read I/O, while in the best case, it can cover 128GB of read I/O. The worst case occurs when the data to be read is fully fragmented into 4KB segments.
